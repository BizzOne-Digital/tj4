...
function allVideosUnder(relativeFolder: string): string[] {
  return listFiles(path.join(DRIVE_ROOT, relativeFolder), relativeFolder)
...
const mainFolder = "MAIN PAGE TAB FOLDER-PICTURES & VIDEOS";
const mainImages = imagesIn(mainFolder);
const mainVideos = allVideosUnder(mainFolder);
...
const manifest = {
  ...
  mainPage: { images: mainImages, videos: mainVideos },
  coaches: {
...
console.log("Summary:", {
  ...
  mainPageVideos: mainVideos.length,
